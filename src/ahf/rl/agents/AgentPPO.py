...
from ahf.rl.agents.net import ActorPPO, ActorDiscretePPO, CriticPPO, SharePPO
# from ahf.rl.train.replay_buffer import ReplayBufferList
...
class AgentPPO(AgentBase):
    ...
    def __init__(
        ...
        """tracker"""
        self.max_step = None
        self.reward_tracker = None  # Tracker(args.tracker_len)
        self.step_tracker = None  # Tracker(args.tracker_len)
    ...
    def explore_one_env(self, env, target_step, random_exploration=None) -> list:
        ...
        self.reward_tracker = Tracker(env.max_step)
        self.step_tracker = Tracker(env.max_step)
        ...
        while step_i < target_step or not done:
            ...
            if getattr(env, 'is_render', False):
                render_mode = getattr(env, 'render_mode', 'console')
    ...
    def explore_vec_env(self, env, target_step, random_exploration=None) -> list:
        ...
        while step_i < target_step or not any(ten_dones):
            ...
            env.render(mode=env.render_mode)
            step_i += 1
    ...
    def update_net(self, buffer):
        ...
        # Step the schedulers after optimization
        current_lr = self.act_optimizer.param_groups[0]['lr']
        print(f"Current learning rates: {current_lr}")

        if current_lr > self.min_learning_rate:
            self.act_scheduler.step()
            if self.cri_scheduler is not self.act_scheduler:

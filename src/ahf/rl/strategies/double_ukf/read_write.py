...
def write_txn_order(
    ...
    try:
        ...
        err_str = readable_error(e, __file__)
        logger.error(f"[write_txn_order] failed err:{err_str}")
...
def strategy_train_sample_params_A(best_params, trial: optuna.Trial):
    pass


def strategy_train_sample_params_B(best_params, trial: optuna.Trial):
    long_level = trial.suggest_uniform("long_level", 0, 0.4)
    long_exit_level = trial.suggest_uniform("long_exit_level", 0, long_level)
    short_level = trial.suggest_uniform("short_level", -0.4, 0)
    short_exit_level = trial.suggest_uniform("short_exit_level", short_level, 0)
    buy_delta = trial.suggest_loguniform("buy_delta", 1e-09, 0.9999)
    sell_delta = trial.suggest_loguniform("sell_delta",  1e-09, buy_delta)

    default_sell_delta_change = trial.suggest_uniform("default_sell_delta_change", 0.05, 0.15),
    sell_delta_limit_upper = trial.suggest_loguniform("sell_delta_limit_upper", 1e-09, 0.7)
    sell_delta_limit_lower = trial.suggest_loguniform("sell_delta_limit_lower", 1e-09, sell_delta_limit_upper*0.9)

    return {
        "long_level": long_level,
        "long_exit_level": long_exit_level,
        "short_level": short_level,
        "short_exit_level": short_exit_level,
        "buy_delta": buy_delta,
        "sell_delta": sell_delta,

        # "buy_obs_cov": trial.suggest_uniform("buy_obs_cov", 0.1, 0.5),
        # "buy_sd_delta": trial.suggest_loguniform("buy_sd_delta", 1e-03, 0.1),
        # "buy_sd_obs_cov": trial.suggest_uniform("buy_sd_obs_cov", 0.1, 0.5),
        # "sell_obs_cov": trial.suggest_uniform("sell_obs_cov", 0.1, 0.5),
        # "sell_sd_delta": trial.suggest_loguniform("sell_sd_delta", 1e-6, 0.5),
        # "sell_sd_obs_cov": trial.suggest_uniform("sell_sd_obs_cov", 0.1, 0.5),

        "default_sell_delta_change": default_sell_delta_change,
        "sell_delta_limit_upper": sell_delta_limit_upper,
        "sell_delta_limit_lower": sell_delta_limit_lower

    }


def strategy_tune_sample_params(sell_delta: float, trial: optuna.Trial):

    default_sell_delta_change = trial.suggest_uniform("default_sell_delta_change", 0.05, 0.15)
    sell_delta_limit_upper = sell_delta

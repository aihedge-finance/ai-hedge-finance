...
class Strategy:
    ...
    def __init__(self, app_env, trade_args, tech_args, _logger):
        ...
        try:
            ...
            assert trade_args.get("stop_loss_margin") is not None, "stop_loss_margin is required for strategy double_ukf"
    ...
    def step(self, price_list: List[Decimal], dt_idx: dt.datetime, ds, long_short,
             buy_delta=None, sell_delta=None) -> Tuple[TradeAction, Tuple]:
        """
...
def _run_sim(_trade_args, _tech_args, plot_args, _logger,
    ...
    # logger = setup_logger("double_ukf_strategy_py.log", trade_args.get("symbol"))
    ...
    try:
        ...
        _logger.info("[Strategy] double_ukf Final Result for step, "
                    "init:${0:>9,.3f}, "
                    "pnl:${1:>9,.3f}, "
                    "gain:{2:>8,.3f}%, "
                    "cal count:{3:>3}"
        ...
        if gain_pct > 15:
            ...
            save_file(f"{project_root}/logs/double_ukf_strategy_RunSim_{v.ds.symbol}_{_trade_args.get('trade_interval')}.txt", json_line, 'a')
            lock.release()
...
def run_ind(form_start, _trade_args, _tech_args, _logger):
    ...
    try:
        ...
        _logger.info(f"[Double_UKF] Double_UKF.Strategy.gen_data.trade_args:{pretty_dict(_trade_args)}")
        ...
        dt_idx_list, prices_list, buy_tracer_list, buy_alfa_list, sell_tracer_list, sell_alfa_list  = [], [], [], [], [], []
        ...
        while True:
            ...
            # 交易規則模型
            buy_ind, sell_ind = strategy.step_ind(prices,
                                                  dt_idx,
                                                  strategy.buy_delta,
                                                  strategy.sell_delta,
                                                  long_short)
            ...
            buy_alfa_list.append(buy_ind.alfa[1])
            sell_tracer_list.append(sell_ind.tracer[1])
            sell_alfa_list.append(sell_ind.alfa[1])
...
def manual_load_param():
    ...
    _trade_args = {"exch_mode": "SpotAPI", "tech_id": "double_ukf", "symbol": "BTCUSDT", "exchange": "Binance",
...
if __name__ == "__main__":
    ...
    configure_logger("double_ukf_strategy_main", False)

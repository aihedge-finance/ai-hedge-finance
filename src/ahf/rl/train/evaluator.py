...
class Evaluator:
    def __init__(self, cwd: str, agent_id: int, eval_env, args: Arguments, logger: None):
        self.recorder = list()  # total_step, r_avg, r_std, obj_c, ...
        self.recorder_path = os.path.expanduser(f'{cwd}/recorder.npy')
    ...
    def evaluate_save_and_plot(self, act, steps: int, r_exp: float, log_tuple: tuple) -> (bool, bool):
        try:
            ...
            print(err) if self.logger is None else self.logger.error(err)
            time.sleep(3)
            sys.exit()
...
def get_cumulative_returns_and_step(env, act) -> (float, int):
    ...
    try:
        ...
        print(f"[evaluator] get_cumulative_returns_and_step device {device}")
...
def get_cumulative_returns_and_step_isaacgym(env, act) -> (float, int):  # [ElegantRL.2022.03.03]
    ...
    if_discrete = env.if_discrete
    device = next(act.parameters()).device  # net.parameters() is a Python generator.
    print(f"[evaluator] get_cumulative_returns_and_step_isaacgym device {device}")
...
def get_rewards_step_list_from_vec_env(env, act) -> list:  # todo
    device = env.device
    print(f"[evaluator] get_rewards_step_list_from_vec_env device {device}")
...
def demo_evaluate_actors(dir_path: str, gpu_id: int, agent, env_args: Dict[str, Any], eval_times=2, net_dim=128):
    import gym
    from elegantrl.train.config import build_env
...
def demo_load_pendulum_and_render():
    ...
    if_discrete = env.if_discrete
    device = next(act.parameters()).device  # net.parameters() is a Python generator.
    print(f"[evaluator] demo_load_pendulum_and_render device {device}")

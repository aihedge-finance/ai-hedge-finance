...
from ahf.preprocessor.tail.tail_C import RSI as RSI_tail


class RSI:
    cols_name = ['idx', 'dt_idx', 'price', 'rsi']
    cols_types = {'idx': int,
                  'dt_idx': str,
                  'price': float,
                  'rsi': float
    ...
    def __init__(self, rsi_time_period, logger, verbose=True):
        """
        REFERENCE: https://indzara.com/2021/04/rsi-technical-indicator-excel-template/

        :param rsi_time_period:
        :param logger:
        :param verbose:
        """
        ...
        self.RSI_tail = RSI_tail(timeperiod=rsi_time_period)

        self.price_pd = None
        self.time_period = rsi_time_period
    ...
    def reset(self):
        ...
        self.rsi = deque(maxlen=self.max_deque)

    def populate_init(self, dt_idx, price, rsi):
        """
    ...
    def add_one(self, price, dt_idx):
        try:
            ...
            # record price
            self.price.append(price)
            tmp = self.RSI_tail.add_one(price)
            ...
            if len(self.rsi) > 1:
                return {"rsi": [self.rsi[-2], self.rsi[-1]]}
            else:
                return {"rsi": [self.rsi[-1], self.rsi[-1]]}
    ...
    def _save_data(self, file_dir: str,
        ...
        try:
            ...
            if self.verbose:
                self.logger.info('[RSI] RSI saved to \n{0}'.format(file_dir))
        except Exception as e:
            err = readable_error(e, __file__)
            self.logger.error(err)
            sys.exit()
